...
namespace BookLibrary.Tests.Services
{
    ...
    {
        private readonly Mock<IBookRepository> _repositoryBookMock;
        private readonly Mock<IMapper> _mapperMock;
        private readonly Mock<IAuthorRepository> _repositoryAuthorMock;

        public BookServiceTests()
        {
            _repositoryBookMock = new Mock<IBookRepository>();
        ...
        public async Task GetAllBooks_ReturnsBookItem()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetAllBooksAsync(cancellationToken))
                                  .ReturnsAsync(tBooks);

            _mapperMock.Setup(m => m.Map<IReadOnlyList<BookItem>>(tBooks))
                      .Returns(bookItems);

            var bookService = new BookService(
                _repositoryBookMock.Object,
                _mapperMock.Object,
                _repositoryAuthorMock.Object
            );
            ...
            _repositoryBookMock.Verify(r => r.GetAllBooksAsync(cancellationToken), Times.Once);
            _mapperMock.Verify(m => m.Map<IReadOnlyList<BookItem>>(tBooks), Times.Once);
        }
        ...
        public async Task GetBookById_ReturnsBookItem()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByIdAsync(1, cancellationToken))
                              .ReturnsAsync(tBook);

            _mapperMock.Setup(m => m.Map<BookItem>(tBook)).Returns(bookItem);
            ...
            _repositoryBookMock.Verify(r => r.GetBookByIdAsync(1, cancellationToken), Times.Once);
            _mapperMock.Verify(m => m.Map<BookItem>(tBook), Times.Once);
        }
        ...
        public async Task GetBookById_ReturnsNull()
        {
            ...
            Assert.Null(result);
            _repositoryBookMock.Verify(r => r.GetBookByIdAsync(1, cancellationToken), Times.Once);
            _mapperMock.Verify(m => m.Map<BookItem>(tBook), Times.Never);
        }
        ...
        public async Task CreateBook_ReturnsBookItem()
        {
            ...
            // Setup: Author exists
            _repositoryAuthorMock.Setup(a => a.GetAuthorByIdAsync(bookItem.AuthorId, cancellationToken))
                                .ReturnsAsync(tAuthor);

            // Setup: No existing book with same title
            _repositoryBookMock.Setup(r => r.GetBookByTitleAsync("New Book", cancellationToken))
                              .ReturnsAsync((TBook?)null);

            // Setup: Map BookItem to TBook
            _mapperMock.Setup(m => m.Map<TBook>(bookItem)).Returns(tBook);

            // Setup: Add, Save, LoadAuthor
            _repositoryBookMock.Setup(r => r.AddBookAsync(It.IsAny<TBook>(), cancellationToken))
                              .Returns(Task.CompletedTask);

            _repositoryBookMock.Setup(r => r.SaveChangesAsync(cancellationToken))
                              .Returns(Task.CompletedTask);

            _repositoryBookMock.Setup(r => r.LoadAuthorAsync(It.IsAny<TBook>(), cancellationToken))
                              .Returns(Task.CompletedTask)
                              .Callback<TBook, CancellationToken>((book, ct) => book.Author = tAuthor);

            // Setup: Map TBook back to BookItem
            _mapperMock.Setup(m => m.Map<BookItem>(It.Is<TBook>(b => b.Title == "New Book")))
            ...
            _repositoryAuthorMock.Verify(a => a.GetAuthorByIdAsync(bookItem.AuthorId, cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.GetBookByTitleAsync("New Book", cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.AddBookAsync(It.IsAny<TBook>(), cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.SaveChangesAsync(cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.LoadAuthorAsync(It.IsAny<TBook>(), cancellationToken), Times.Once);
            _mapperMock.Verify(m => m.Map<TBook>(bookItem), Times.Once);
            _mapperMock.Verify(m => m.Map<BookItem>(It.IsAny<TBook>()), Times.Once);
        }
        ...
        public async Task CreateBook_ThrowsInvalidOperationException_WhenAuthorNotFound()
        {
            ...
            _repositoryAuthorMock.Setup(a => a.GetAuthorByIdAsync(999, cancellationToken))
        ...
        public async Task CreateBook_ThrowsInvalidOperationException_WhenBookTitleAlreadyExists()
        {
            ...
            _repositoryAuthorMock.Setup(a => a.GetAuthorByIdAsync(1, cancellationToken))
                                .ReturnsAsync(tAuthor);

            _repositoryBookMock.Setup(r => r.GetBookByTitleAsync("Existing Book", cancellationToken))
        ...
        public async Task UpdateBook_ReturnsBookItem()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByIdAsync(bookId, cancellationToken))
                              .ReturnsAsync(existingTBook);

            _repositoryBookMock.Setup(r => r.GetBookByTitleAsync("Updated Title", cancellationToken))
                              .ReturnsAsync((TBook?)null);

            _repositoryAuthorMock.Setup(a => a.GetAuthorByIdAsync(2, cancellationToken))
                                .ReturnsAsync(author);
            ...
            _repositoryBookMock.Setup(r => r.LoadAuthorAsync(existingTBook, cancellationToken))
                              .Returns(Task.CompletedTask)
                              .Callback<TBook, CancellationToken>((book, ct) => book.Author = author);

            _mapperMock.Setup(m => m.Map<BookItem>(It.Is<TBook>(b => b.Id == bookId && b.Title == "Updated Title")))
                      .Returns(resultBookItem);
            ...
            _repositoryBookMock.Verify(r => r.GetBookByIdAsync(bookId, cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.GetBookByTitleAsync("Updated Title", cancellationToken), Times.Once);
            _repositoryAuthorMock.Verify(a => a.GetAuthorByIdAsync(2, cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.SaveChangesAsync(cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.LoadAuthorAsync(existingTBook, cancellationToken), Times.Once);
            _mapperMock.Verify(m => m.Map<BookItem>(It.IsAny<TBook>()), Times.Once);
        }
        ...
        public async Task UpdateBook_ThrowsInvalidOperationException_WhenBookNotFound()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByIdAsync(999, cancellationToken))
                              .ReturnsAsync((TBook?)null);
        ...
        public async Task UpdateBook_ThrowsInvalidOperationException_WhenTitleAlreadyExists()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByTitleAsync("New Title", cancellationToken))
        ...
        public async Task UpdateBook_ThrowsInvalidOperationException_WhenAuthorNotFound()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByTitleAsync("Title", cancellationToken))
                              .ReturnsAsync((TBook?)null);
        ...
        public async Task DeleteBookById_ReturnsTaskCompleted()
        {
            ...
            _repositoryBookMock.Setup(r => r.GetBookByIdAsync(bookId, cancellationToken))
                                .ReturnsAsync(existingTBook);
            _repositoryBookMock.Setup(r => r.DeleteBookById(existingTBook, cancellationToken))
                                .Returns(Task.CompletedTask);
            _repositoryBookMock.Setup(r => r.SaveChangesAsync(cancellationToken))
                                .Returns(Task.CompletedTask);
            ...
            // Assert
            _repositoryBookMock.Verify(r => r.GetBookByIdAsync(bookId, cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.DeleteBookById(existingTBook, cancellationToken), Times.Once);
            _repositoryBookMock.Verify(r => r.SaveChangesAsync(cancellationToken), Times.Once);
        }
